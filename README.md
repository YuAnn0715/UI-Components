...
本專案只維護 standalone 靜態檔案流程；使用者只需要下載元件包，再引用 Bootstrap Icons、`ui-components.css` 與 `ui-components.js`。
...
1. 從展示站下載 `UiComponentLibrary-static.zip`，或直接複製 `standalone` 中的 `ui-components.css`、`ui-components.js` 與 `bootstrap-icons/` 資料夾。
2. 將兩個元件檔案與 `bootstrap-icons/` 資料夾放入目標網站的 `wwwroot/lib/ui-components/`，並保留圖示字型的目錄結構。
3. 在 Layout 先載入 Bootstrap Icons，再載入元件 CSS，並在元件 HTML 後載入 JavaScript：
