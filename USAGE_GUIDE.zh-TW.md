...
## 3. 標籤式輸入：`ui-tag-select`

`ui-tag-select` 可以從既有選單選取項目，也可以在輸入框輸入文字後按 `Enter` 建立自訂 tag。每個 tag 的 × 按鈕可以移除值；實際送出的值放在具有 `name` 與 `multiple` 的隱藏 `select`。
...
<div class="ui-field ui-tag-select-field"
  ...
  <div class="ui-tag-select-control" data-ui-tag-select-control>
    ...
    <input class="ui-tag-select-input"
           id="Skills_input"
           type="text"
           placeholder="輸入技能後按 Enter 新增"
           data-ui-tag-select-input
           autocomplete="off" />
...
按下 `Enter` 後，輸入文字會被 trim；空白與重複值不會建立 tag。自訂值會動態加入 `data-ui-tag-select-values`，因此可直接由 ASP.NET Core 的 `Request.Form["Skills"]` 取得。
